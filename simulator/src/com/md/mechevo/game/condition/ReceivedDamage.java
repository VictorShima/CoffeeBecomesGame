...
public class ReceivedDamage extends Condition {
    ...
    public boolean check(State state) {
        //We add 180 degrees to the angle because the bullet travels in the opposite direction than the players angle
        double hitAngle = this.getOwner().getLastHitAngle();
        double playerAngle = this.getOwner().getAngle() + 180f;
        if((hitAngle > (this.getFace().getMinAngle() + playerAngle)%360f) && (hitAngle < (this.getFace().getMaxAngle() + playerAngle)%360f)){
