...
public class BatteringRam extends Weapon {
	...
	public void fire(State state, Solid target) {
		if(target != null){
			...
			Projectile proj =
					new MeleeProjectile(state.getNextId(), this.getPlayer().getPosition(), this.getPlayer().getAngle() + angle, this);
			state.addProjectile(proj);
		}else{
			Projectile proj =
					new MeleeProjectile(state.getNextId(), this.getPlayer().getPosition(), this.getPlayer().getAngle(), this);
			state.addProjectile(proj);
		}
	}
}
